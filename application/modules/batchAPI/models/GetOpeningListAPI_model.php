...
class GetOpeningListAPI_model extends CI_Model
{
	...
	public function callOpeningDetailsAPI($tokenData,$limitSize){
		...
		$jdata = '{"buIdList":[],"cvSourceIdList":[],"statusIdList":[1,2,3,4,5],"pagination":{"start":'.$limitSize.',"limit":500}}';
		...
		if (curl_errno($ch))
		{
			...
			var_dump("error");
			$this->ApiLog_model->insertApiLog("Opening List", $url, curl_error($ch), false);
			return false;
		}
		else
		{						
			...
			curl_close($ch);
			$resString = $transaction;
			$this->ApiLog_model->insertApiLog("Opening List", $url, "", true);
			if(!empty($transaction) && !isset($transaction['error'])){
				...
				echo "<br>Opening Details Added To Table for limit :".$limitSize." - 500";
			}else
			{
				echo "<br> No Data Found for limit : ".$limitSize." - 500";
				if($limitSize == 0)
