...
class RecruiterJobDetails_model extends CI_Model
{
    ...
    public function getRecruiterJobLogs()
    {
        ...
        $SQL = "select jrs.recruiterId, r.company_name as recruiterName, jrs.openingId, o.openTitle, o.statusId, o.statusName, jrs.postedResumes, jrs.maxCount, DATE_FORMAT(jrs.updatedDate, '%c-%b-%y %l:%i %p') as updatedDate from jobid_recruiter_submissions jrs
    ...
    public function updateMaxCount($openingId, $oldMaxCountRecruiter, $newMaxCountRecruiter, $maxCount, $fixedShare, $bounsShare, $fixedShareInPerc, $bounsShareInPerc, $userName)
    {
        $SQL = "update opening_details set max_count = " . $maxCount . ", max_per_recruiter = " . $newMaxCountRecruiter . ", fixedShare = " . $fixedShare . ", bonusShare = " . $bounsShare . ", fixedShareInPerc = " . $fixedShareInPerc . ", bonusShareInPerc = " . $bounsShareInPerc . ", updatedDate = now() where openingId = " . $openingId;
        $query = $this->db->query($SQL);
    ...
    // {
    //     $sql = "insert into ratecard_buid_mapping(openingId, buId, fromDate, tillDate, fixedShare, fixedShareInPerc, bonusShare, bonusShareInPerc, createdBy, updatedBy) values(" . $openingId . ", " . $buId . ",'" . $fromDate . "','" . $tillDate . "', " . $fixedShare . ", " . $fixedShareInPerc . ", " . $bonusShare . ", " . $bonusShareInPerc . ", '" . $createdBy . "', '" . $createdBy . "')";
    //     $query = $this->db->query($sql);
    //     return true;
    // }
    ...
    public function insertRateCardForVendor($openingId, $buId, $fromDate, $tillDate, $fixedShare, $fixedShareInPerc, $bonusShare, $bonusShareInPerc, $createdBy)
    {
        ...
        if ($bonusShare > 0 || $bonusShareInPerc > 0) {
            ...
            if ($openingId > 0) {
                $jobdet_qry = $jobdet_qry . " and o.openingId = " . $openingId;
            }
            if ($buId > 0) {
                $jobdet_qry = $jobdet_qry . " and o.buId = " . $buId;
            }
            $jobdet = $this->db->query($jobdet_qry)->result();
    ...
    public function deleteRateCardForVendor($id)
    {
        ...
        $ratedet = $this->db->query($sql)->result();
        
        for ($j = 0; $j < sizeof($ratedet); $j++) {
            $jobdet_qry = "select o.id from opening_details o where o.statusId=3 ";
            if ($ratedet[$j]->openingId > 0) {
                $jobdet_qry = $jobdet_qry . " and o.openingId = " . $ratedet[$j]->openingId;
            }
            if ($ratedet[$j]->buId > 0) {
                $jobdet_qry = $jobdet_qry . " and o.buId = " . $ratedet[$j]->buId;
            }
            $jobdet = $this->db->query($jobdet_qry)->result();
            for ($i = 0; $i < sizeof($jobdet); $i++) {
                $del_qry = "delete from job_category_details where converted_by = 'A' and job_id = ".$jobdet[$i]->id." and convert(start_date, DATE) = convert('".$ratedet[$j]->fromDate."', DATE) and end_date = convert('".$ratedet[$j]->tillDate."', DATE)";
